...
import { AirGapNodeClient } from '../../src/v1'
...
const protocols: TestProtocolSpec<string>[] = [new RootstockTestProtocolSpec(), new ERC20TokenTestProtocolSpec()]
...
Promise.all(
  protocols.map(async (protocol: TestProtocolSpec<string>) => {
    ...
    describe(`Protocol ${protocol.name}`, () => {
      describe(`Public/Private KeyPair`, () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.wallet.extendedSecretKey && protocol.wallet.extendedSecretKey,
          'getExtendedKeyPairFromDerivative - should be able to create an extended key pair from a derivative (extended keys)',
          async () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.wallet.extendedPublicKey,
          'deriveFromExtendedKey and getAddressFromPublicKey - should be able to derive a public key from an extended public key and derive an address from it',
          async () => {
            if (!isBip32Protocol(protocol.lib) || !protocol.wallet.extendedPublicKey) {
              // check in `itIf` doesn't trigger autocasting
              return
            }
      ...
      describe(`Prepare Transaction`, () => {
        ...
        itIf(!isBip32Protocol(protocol.lib), 'prepareTransactionWithPublicKey - Is able to prepare a tx using its public key', async () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.wallet.extendedPublicKey,
          'prepareTransactionWithPublicKey - Is able to prepare a tx using its extended public key',
          async () => {
            if (!isBip32Protocol(protocol.lib) || !protocol.wallet.extendedPublicKey) {
              // check in `itIf` doesn't trigger autocasting
              return
        ...
        itIf(
          !isBip32Protocol(protocol.lib),
          'prepareTransactionWithPublicKey - Is able to prepare a transaction with amount 0',
          async () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.wallet.extendedPublicKey,
          'prepareTransactionWithPublicKey - Is able to prepare a transaction (with its extended public key) with amount 0',
          async () => {
            if (!isBip32Protocol(protocol.lib) || !protocol.wallet.extendedPublicKey) {
              // check in `itIf` doesn't trigger autocasting
              return
      ...
      describe(`Sign Transaction`, () => {
        ...
        itIf(!isBip32Protocol(protocol.lib), 'signTransactionWithSecretKey - Is able to sign a transaction using a SecretKey', async () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib),
          'signTransactionWithSecretKey - Is able to sign a transaction using an ExtendedSecretKey',
          async () => {
            if (!isBip32Protocol(protocol.lib)) {
              // check in `itIf` doesn't trigger autocasting
              return
      ...
      describe(`Extract TX`, () => {
        itIf(
          !isBip32Protocol(protocol.lib),
          'getDetailsFromTransaction - Is able to extract all necessary properties from an unsigned TX',
          async () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.wallet.extendedPublicKey,
          'getDetailsFromTransaction - Is able to extract all necessary properties from an unsigned TX (with an extended public key)',
          async () => {
            if (!isBip32Protocol(protocol.lib) || !protocol.wallet.extendedPublicKey) {
              // check in `itIf` doesn't trigger autocasting
              return
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.wallet.extendedPublicKey,
          'getDetailsFromTransaction - Is able to extract all necessary properties from a signed TX',
          async () => {
            if (!isBip32Protocol(protocol.lib) || !protocol.wallet.extendedPublicKey) {
              // check in `itIf` doesn't trigger autocasting
              return
        ...
        it('getTransactionStatus - Is able to get transaction status', async () => {
          ...
          for (let i = 0; i < tests.length; i++) {
            ...
            const statuses: Record<string, AirGapTransactionStatus> = await protocol.lib.getTransactionStatus(Object.keys(tests[i]))
            sinon.restore()
            expect(statuses, 'transactionStatus').to.deep.equal(tests[i])
          }
        })
      })
      ...
      describe(`Sign Message`, async () => {
        ...
        itIf(
          ...
          'signMessageWithKeyPair - Is able to sign a message using a key pair',
          async () => {
            const { secretKey, publicKey } = await protocol.lib.getKeyPairFromDerivative(await protocol.derivative())
        ...
        itIf(
          !isBip32Protocol(protocol.lib) && protocol.messages.length > 0,
          'verifyMessageWithPublicKey - Is able to verify a message using a PublicKey',
          async () => {
            const { publicKey } = await protocol.lib.getKeyPairFromDerivative(await protocol.derivative())
            for (const messageObject of protocol.messages) {
              try {
        ...
        itIf(
          ...
          async () => {
            ...
            const { publicKey } = await protocol.lib.getExtendedKeyPairFromDerivative(await protocol.derivative())
      ...
      describe(`Encrypt Message Asymmetric`, () => {
        ...
        itIf(
          isBip32Protocol(protocol.lib) && protocol.encryptAsymmetric.length > 0,
          'decryptAsymmetricWithKeyPair - Is able to decrypt a message using an ExtendedSecretKey',
          async () => {
            if (!isBip32Protocol(protocol.lib)) {
              // check in `itIf` doesn't trigger autocasting
              return
      ...
      describe(`Encrypt Message Symmetric`, () => {
        ...
        itIf(
          ...
          'decryptAESWithSecretKey - Is able to encrypt a message using a SecretKey',
          async () => {
            const { secretKey } = await protocol.lib.getKeyPairFromDerivative(await protocol.derivative())
